...
function ProductList(){
  ...
  const loadUser = async () => {
    const result = await axios.get("http://localhost:3999/products");
    // console.log(result);
    setUsers(result.data);
  }

  const deleteUser = async (id) => {
    await axios.delete(`http://localhost:3999/products/${id}/`);
  ...
  return (
    <div className='container' style={{display: "block", background: "none"}}>
      ...
      <Link to="/addingProduct" className="btn btn-outline-primary">Add product</Link>
      </div>
      <Table>
        <thead>
          <tr>
        ...
        {users.map((user, index) => (
          <tr>
            ...
            <td>
              ...
              <Link className='btn btn-danger m-1' onClick={() => {deleteUser(user.pid)}}><i class="fa fa-trash" aria-hidden="true"></i></Link>
            </td>
          </tr>
        ))}
        </tbody>
